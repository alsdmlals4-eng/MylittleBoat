...
Automated contracts cover foreground cadence including a valid zero-event five-minute voyage, no input/no together-time side effect, malformed ConfigFile values, named writer isolation, and scene-to-storage restore. The earlier controlled bright-lagoon capture is historical. The current receipt is six controlled 540×960 GPU pass captures in `docs/evidence/2026-08-31-ambient-scenery-pass/` for `MLB-AMB-MOTIF-001..006`, one per approved local-time landscape. They show the motif moving while water-only base backdrops stay active. These do not prove Human five-minute calm, noticeability, text readability, or device comfort.
