...
`my little boat`의 최상위 감정 목표는 **플레이어가 무언가를 달성해서 만족하는 것보다, 작은 보트에서 내 캐릭터와 펫이 목적지 없이 잔잔한 바다를 함께 지나며 쉬었다고 느끼게 하는 것**입니다.
...
**작고 둥근 보트 디오라마에서 내 캐릭터와 동반자가 목적지 없이 잔잔한 바다·파도소리와 함께 천천히 지나가며, 아무것도 하지 않아도 편안한 시간을 보내는 게임.**
...
첫 화면은 설명·선택·확인 절차가 아니라 이미 떠 있는 보트입니다.

```text
실행
→ Avatar + 동반자 + 보트 + 바다 + 수평선이 함께 보이는 Normal 3/4 디오라마
→ 그냥 머무르기 또는 원할 때만 사진·낚시·감상·작은 상호작용
→ 원할 때만 꾸미기에서 외형·동반자·보트 장식 변경
→ 계속 쉬기 또는 개인적인 기억 남기기
```

- `오늘의 마음`은 제품의 시작 선택이 아닙니다. 이를 다른 질문이나 시작 버튼으로 대체하지 않습니다.
- 기기의 현지 현실 시간이 새벽·밝음·해질녘·밤을 자동으로 정합니다. 수동 분위기 control과 saved atmosphere는 없으며, 기기 시계는 시각 표현 외의 보상·진행·저장에 쓰지 않습니다.
- active foreground 시간에 따라 먼 구조물과 주변 풍경이 낮은 밀도로 흘러갑니다. 이것은 과제·보상 track, 목적지, 도착 보상, 놓친 이벤트 패널이 아닙니다.
- 외형·동반자·장식은 바다를 본 뒤에만 원하면 여는 `꾸미기`의 순수 cosmetic 선택입니다.
- 현재 code에는 이전 선택형 `main_menu`와 mood data가 남아 있습니다. 이는 이 경험 정본을 반박하지 않으며, 다음 Phase 2에서 교체할 `PRODUCT_SUPERSEDED_IMPLEMENTATION`입니다.
...
- 보트의 hull은 waterline, 느린 bob, 주변 잔물결·wake, 반사 또는 가림을 통해 바다와 한 공간으로 읽혀야 한다. 보트가 물 위에 붙여진 이미지처럼 보이는 구성은 main entry로 사용하지 않는다.
...
Normal Diorama와 Appreciation Camera 모두에서 같은 의미로 나타나되, 첫 기회는 foreground 90–150초에 예약하고 매 기회는 65% 확률로만 작은 장면을 표시합니다. 표시하지 않은 경우에도 다음 기회는 120–180초 뒤에 조용히 예약됩니다. 이 수치나 대기 시간은 UI에 나타나지 않으며, 감상 화면의 바다·수평선을 가리지 않아야 합니다. 명목상 5분 항해에는 대체로 1~2회를 목표로 하지만 첫 발견은 보장하지 않고, 0회인 항해도 정상입니다. 병편지·외부 메시지·소셜 알림과 구분하며, 호감도·보상·점수·희귀도·timer·camera·FOMO를 만들지 않습니다. 현재 자동 저장된 풍경 문자열만 `user://ambient_memory_v1.cfg`에 복원 가능하게 남기며, motif 확장·Human five-minute noticeability는 별도 검증 전입니다.
...
함께한 시간은 활성 foreground 항해의 경과 시간만으로 쌓입니다. Normal Diorama와 Appreciation Camera, 그리고 항해 기록 뒤에 더 머무는 시간은 같은 동행 시간으로 인정합니다. 실제 1초는 함께한 시간 1초로 누적되며 입력·사진·풍경·편지·물고기·낚시·발견·꾸미기·상호작용·시간대·속도는 수치의 원천이나 배율이 아닙니다.

메인 메뉴·앨범·백그라운드/일시정지는 누적하지 않습니다. 시간은 `user://together_time_v1.cfg`에 local-only로 저장하며, 15초 단위와 foreground 이탈·Album 전환·scene exit에서 flush합니다. 과거 synthetic level은 실제 시간으로 환산하지 않고 0부터 시작합니다. 이 기록은 관리, 방치 벌, streak, 능력치, 경제, 경쟁, 소셜 자격을 만들지 않습니다.

고양이·토끼·강아지·수달은 순수 local cosmetic 선택이므로, 함께한 시간은 pet type별 ledger가 아닌 전역 기록입니다. 외형을 바꿔도 시간은 잃거나 초기화되지 않고 종별 보정도 없습니다.

### 조용한 표시 원칙

함께한 시간은 항해 화면을 점유하는 level loop가 아니라 앨범에서 돌아보는 동행 기록입니다. 현재 구현은 새 companion Scene 없이 기존 AlbumView에 분 단위 `함께한 시간`과 비경쟁적 관계 문구 한 줄을 둡니다. TopPanel·Appreciation Camera에는 숫자·레벨·진행 바·성장 알림을 두지 않으며, 문구는 reward·unlock·다음 행동을 암시하지 않습니다. 실제 기기에서 이 문구가 편안하게 읽히는지는 `NOT_RUN`입니다.
