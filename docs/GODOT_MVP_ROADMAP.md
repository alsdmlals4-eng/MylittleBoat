...
3. Ambient Discovery와 낚시가 체크리스트가 아니라 작은 발견으로 느껴지는가.
4. 감상모드와 조작 UI가 바다 감상을 방해하지 않는가.
5. 앨범/동반자가 다음 항해의 이유가 되는가.
6. 모바일 세로 화면에서 손가락 입력과 텍스트 가독성이 충분한가.
