...
- **Current 2026-08-31 split-sky/sea receipt:** `all=56, headless=55, display_only=1`이다. 새 `test_split_sky_sea_background_contract.gd`는 세 카메라의 고정 `SkyBackdrop`·흐르는 `SeaBackdrop`, 네 시간대의 실제 asset pair, 동일 flow offset을 확인한다. `test_headless_editor_plugin_guard_contract.gd`는 headless import가 Hera 보조 플러그인의 server/UI resource를 만들기 전에 반환하는지 확인한다. Windows OpenGL capture와 프레임 비교 증거는 `docs/evidence/2026-08-31-split-sky-sea-background/`가 current owner다. 최신 55개 headless 집합은 로컬 Godot 4.7.2에서 exit 0으로 다시 실행했고, import도 ObjectDB/resource leak 경고 없이 exit 0으로 끝났다. 원격 CI와 Human/device comfort는 각각 `NOT_RUN`이다.
