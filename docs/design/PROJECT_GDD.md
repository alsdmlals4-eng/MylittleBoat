...
| 선체-수면 접점 | `PARTIAL_IMPLEMENTED / RUNTIME_CAPTURE_VERIFIED` | existing ripple은 보트의 x/z 및 거의 같은 y bob을 따라가지만, 큰 ring 원화만으로 선체 하단의 접지감이 충분한지는 해결 완료가 아님. 새 waterline contact raster는 `GENERATED_CANDIDATE`이며 user `LOCK` 전까지 runtime 미소비 |
