...
2026-09-20 후속 진행으로 첫 섬 설계는 상세 계획 기준으로 채택됐지만 아래 아트 요구사항은 여전히 제작 전이다. 사용자가 추가한 `MLB-BOAT-OUTING-01`은 나중에 섬 밖을 구경하는 방향이다. 배·부두·바깥 경관의 신규 모델/이미지를 IV01–IV07에 자동 추가하지 않으며, 나들이 명세와 실제 consumer가 정해질 때 별도 상태군으로 연결한다. 현재 승인 보트 자산은 그때 재사용 적합성을 검토할 자료로 보존한다.
