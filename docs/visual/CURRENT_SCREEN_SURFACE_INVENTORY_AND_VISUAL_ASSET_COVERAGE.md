...
| --- | --- | --- | --- | --- | --- |
| `MLB-VC-001` | `MLB-SCR-001`, `MLB-OVR-002` | First impression, identity/light selection; normal, selected, expanded | `GODOT_UI`, `TEXT_LAYER`, `EXISTING_APPROVED` | `main_menu.tscn`, `main_menu.gd`; identity and time catalogs | `REQUIREMENT_LINKED` to `MLB-UI-P0-MAIN-ENTRY-COMPOSITION`. No new image file. |
...
No `RASTER_IMAGE`, `SPRITE_SHEET`, or UI icon requirement is newly queued. The audit found no valid automatic image-generation trigger.
...
- Compose `MLB-SCR-001` with existing approved boat/sea/identity visual anchors and Godot UI/text layers.
- Keep player/pet/time selection, mood meaning, local-first state, 540×960 layout, and direct `main_menu → game` route unchanged.
- Do not create a static menu background PNG, UI icon pack, portrait-only asset family, new progression, save-slot system, or a combat/system screen.
