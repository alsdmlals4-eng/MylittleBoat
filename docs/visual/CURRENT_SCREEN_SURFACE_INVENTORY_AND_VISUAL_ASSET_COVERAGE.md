...
Status: `CANONICAL_VISUAL_COVERAGE_OWNER / 2026-08-27`  
Issue: [#68](https://github.com/alsdmlals4-eng/MylittleBoat/issues/68)  
Scope: current `main` vertical slice only. No image has been generated by this audit.
...
| P0 main entry composition | `GAP_BLOCKING` | Functional screen exists, but its ColorRect/default-panel presentation does not carry the approved first-impression visual identity. This is a Godot UI/composition slice, not an image-generation request. |
...
| New runtime bitmap requirement | `0` | Existing approved boat, sea, identity, cushion, and postcard files cover every current bitmap consumer. |
...
| --- | --- | --- | --- | --- | --- | --- |
| `MLB-SCR-001` / Main entry | `CURRENT_VERTICAL_SLICE` / P0 / `GAME_RUNTIME` | “What kind of quiet place is this, and how do I begin?” | launch or next voyage → mood choice | `scenes/main_menu.tscn`, `scripts/ui/main_menu.gd`; [540×960 capture](../evidence/2026-08-27-screen-surface-audit/main-menu_540x960.png); four approved 1024×1536 atmosphere backgrounds | Notion Home + Visual Bible + Asset Library; this owner + `main_menu.tscn` | `GAP_BLOCKING`. Current `ColorRect + PanelContainer + Label/Button` is readable but lacks the approved boat/sea/companion first impression. Required mode: `GODOT_UI + RASTER_IMAGE + EXISTING_APPROVED`; backgrounds are ready but not yet wired. |
...
| --- | --- | --- | --- | --- | --- |
| `MLB-VC-001` | `MLB-SCR-001`, `MLB-OVR-002` | First impression, identity/light selection; normal, selected, expanded; Dawn/Bright/Sunset/Night | `GODOT_UI`, `TEXT_LAYER`, `RASTER_IMAGE`, `EXISTING_APPROVED` | `main_menu.tscn`, `main_menu.gd`; identity/time catalogs; `assets/images/ui/main_menu/main_menu_*_storybook_v1.png` | `ASSET_READY_RUNTIME_WIRING_PENDING` for `MLB-UI-P0-MAIN-ENTRY-COMPOSITION`. |
...
| `MLB-ASSET-MAIN-ENTRY-UI` | `MLB-SCR-001`, `MLB-OVR-002` | `main_menu.tscn` | normal / selected / expanded identity and time choice | `GODOT_UI`, `TEXT_LAYER`, `REUSE_PROJECT` | 540×960 runtime capture, semantic button tests, later real-device check | `REQUIREMENT_LINKED` |
| `MLB-ASSET-MAIN-MENU-ATMOSPHERE-BACKGROUND` | `MLB-SCR-001`, `MLB-OVR-002` | P0 `main_menu.tscn` TextureRect background, selected by the existing four-time catalog | Dawn / Bright / Sunset / Night, opaque portrait 2:3 | `RASTER_IMAGE`, `USER_APPROVED`, `GENERATION_PROVENANCE` | four exact 1024×1536 RGB sRGB PNGs; SHA-256, Notion records, and durable Git locators in `docs/evidence/2026-08-28-main-menu-background-assets/asset-provenance.md` | `IMPLEMENTATION_READY_ASSET / RUNTIME_WIRING_PENDING` |
...
| Main entry is functional but visually generic. | It does not present the approved hand-painted boat-rest identity before the player commits to a mood. | Create a P0 Godot UI handoff with the four approved atmosphere backgrounds behind the existing boat/identity composition. | First-launch mood selection. | `main-menu_540x960.png`, `assets/images/ui/main_menu/`, current `main_menu.tscn`, Notion Home visual canon. |
...
```text
/goal Implement GitHub Issue #68 exactly as specified.
```
