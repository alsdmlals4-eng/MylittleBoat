...
| `MLB-AMB-SEASONAL-ISLAND-001` | Bright/spring `SeasonalIslandLayer`, Normal·Appreciation의 existing ambient-scene transit 위. 원본의 투명 캔버스는 runtime `region_rect`로 제외하고, 수평선의 원거리 landmark만 보인다 | `USER_APPROVED → CANON_REGISTERED → ASSET_READY → IMPLEMENTED → MACHINE_VERIFIED → RUNTIME_CAPTURE_VERIFIED`; alpha source/canonical SHA equality, headless contract, 2026-09-01 OpenGL two-view capture verified. Human `NOT_RUN` |
