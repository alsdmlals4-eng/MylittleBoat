...
**2026-09-16 방향 변경.** 앞으로는 작은 섬에서 농장을 돌보고 바다를 바라보는 힐링 게임을 만들며, 아트는 일본 청춘 애니메이션풍으로 검토합니다. 새 섬 농장 구현은 아직 없습니다. 아래 설명과 실행 방법은 보존된 보트 버전 기준입니다. 현재 결정은 [GDD](docs/design/PROJECT_GDD.md), 마감·날짜별 누적 기록은 [현재 handoff](docs/handoffs/CURRENT_GODOT_IMPLEMENTATION.md)를 따릅니다. 저장 데이터·기존 자산은 삭제하지 않았습니다.
...
2026-09-13 경로 연결 작업은 `VoyageRoute`의 실제 Path3D 위치를 배·세 카메라·두 수면 접점에 공유합니다. 128-unit 직선 구간을 이어 이전 512-unit 경계의 역이동을 없앱니다. 현재 계약 집합은 **67개 중 66개 headless**이며 설정 저장 복구, owner별 복구, 저장 성공 뒤 상태 확정, 사진 저장·경로 안전 계약을 포함합니다. 이는 검사 대상 집합이며 전체 실행 결과는 별도 증거를 확인해야 합니다. 공통 공간의 이동 기반이 있지만 world 섬·연속 3D 모델·새 수면 완성을 뜻하지 않습니다. 구현·검증 단계는 [현재 handoff](docs/handoffs/CURRENT_GODOT_IMPLEMENTATION.md)를 따릅니다.

쉬는 메뉴의 `바다 소리`에서 끄기·25·50·75·100%를 선택합니다. 100%는 기존 조용한 믹스이며 추가 증폭하지 않습니다. 선택은 기존 `comfort_preferences_v1.cfg`에 움직임 설정과 함께 보존되고, 음량을 바꿔도 바다 소리를 처음부터 재생하지 않습니다. 저장 실패 시 현재 실행의 음량은 바꾸되 실패 문구를 표시합니다. 실제 기기와 사람의 청취 편안함은 별도 검증입니다.
...
- 사진을 누르면 현재 항해의 UI 없는 실제 렌더 프레임이 `user://voyage_postcards_v1.cfg`와 로컬 PNG로 저장됩니다. 앨범은 최근 세 장부터 열고 `이전 사진`/`최근 사진`으로 세 장씩 넘깁니다. 작은 화면은 내용을 스크롤하며 `바다로 돌아가기`는 고정됩니다. 파일이 없으면 제목과 불러오기 불가 상태를 남기고 원본·기록을 자동 삭제하지 않습니다. 점수·보상·공유는 없습니다.
...
블루프린트와 별개인 월간 증빙 PDF는 `tools/build_ai_work_evidence_pdf.py`로 발행합니다. 2026년 9월 사용 예는 `python tools/build_ai_work_evidence_pdf.py --output-dir "C:/Users/user/Documents/증빙서류/9월 증빙서류" --version 1.1 --change-reason "실제 추가 또는 정정 내용"`입니다. 새 발행 전 코드 안의 사례 본문을 현재 evidence owner와 대조·갱신해야 합니다. 날짜별 Git 색인과 hash만 자동 수집하며 모든 AI 사용 이력이나 미래 작업을 자동으로 판정하지 않습니다. 같은 버전 파일은 덮어쓰지 않습니다. PDF와 `.sources.json`을 함께 보관하고 최종 PDF의 모든 페이지를 렌더 검수합니다. 실제 작업일·사후 정리일·캡처일·발행일과 미확인 계정·입력 화면·결제 증빙을 구분하며 자동 제출하지 않습니다.
