...
def main():
    ...
    add('상세 기록 안내. 3쪽 이미지 후보 / 4쪽 수면 방향 교정 / 5쪽 저장 보호 / 6쪽 이번 지시 반영 / 7쪽 증빙 공백 / 8쪽 이후 원본 색인.', small)
    ...
    add('AI 서비스별 찾아보기. Codex 작업 검토는 4~6쪽. 이미지 모델 프롬프트와 후보 결과는 3쪽. 과거 생성 모델의 정확한 서비스/계정은 manifest만으로 확인할 수 없으므로 미확인입니다. 결제 내역은 이 보고서의 근거로 검토하지 않았습니다.', small)
    ...
    add('남은 연결', sub)
    add('사진 목록/PNG 저장과 경로 경계, GameState의 저장 성공 후 상태 확정, 플레이어용 복구 안내 UI는 아직 완료되지 않았습니다. R07b2 하위 작업은 사용량 제한으로 중단되었으며 새 구현 커밋이 확인되지 않았습니다. 사용자 세이브를 삭제하거나 자동 초기화하지 않았습니다.')
