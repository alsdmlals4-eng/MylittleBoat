...
func _run() -> void:
	...
	if final_card != null:
		...
		if final_card.texture != null:
			_expect(final_card.texture is ViewportTexture, "default card must render approved independently animated parts")
		_expect(final_card.material_override == null, "RGBA parts must not pass through the old green chroma shader")
